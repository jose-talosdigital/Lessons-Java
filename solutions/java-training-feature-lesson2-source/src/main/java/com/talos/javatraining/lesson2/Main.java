...
public class Main
{
	...
	protected Measure<String> makeItReadable(Measure<BigDecimal> measure)
	{
		 ...
		 */
		setAttributes(measure::getHumidity,result::setHumidity,"%");
		setAttributes(measure::getPressure, result::setPressure, "pa");
		setAttributes(measure::getWindSpeed, result::setWindSpeed, "mph");
		setAttributes(measure::getTemperature, result::setTemperature, "°C");

		return result;
	}

	protected void setAttributes(Supplier<BigDecimal> measure, Consumer<String> result, String unit){
		result.accept(makeItReadable(measure.get(),unit));
	}
